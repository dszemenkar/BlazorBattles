...
namespace BlazorBattles.Server.Controllers
{
    ...
    public class BattleController : BaseApiController
    {
        ...
        public async Task<IActionResult> StartBattle([FromBody] int opponentId)
        {
            ...
            var opponent = await _context.AppUsers.FindAsync(opponentId);
        ...
        private async Task Fight(AppUser attacker, AppUser opponent, BattleResult result)
        {
        ...
        private int FightRound(AppUser attacker, AppUser opponent,
        ...
        private async Task FinishFight(AppUser attacker, AppUser opponent, BattleResult result,
            int attackerDamageSum, int opponentDamgeSum)
        {
            result.AttackerDamageSum = attackerDamageSum;
            result.OpponentDamageSum = opponentDamgeSum;
            ...
            if (result.IsVictory)
            {
                attacker.Victories++;
                opponent.Defeats++;
                attacker.Gold += opponentDamgeSum;
                opponent.Gold += attackerDamageSum * 10;
            }
            else
            {
                attacker.Defeats++;
                opponent.Victories++;
                attacker.Gold += opponentDamgeSum * 10;
                opponent.Gold += attackerDamageSum;
            }
        ...
        private void StoreBattleHistory(AppUser attacker, AppUser opponent, BattleResult result)
        {
